...
* */
case class Fn_Update(updatedResource: Resource, computation: List[SlanExpression]) extends SlanFunction
/*
...
* */
case class Fn_Remove(structure4Removal: Resource, specificValues2remove: Option[List[SlanExpression]]) extends SlanFunction
...
* */
case class Fn_Create(generator: Generator | List[(Message, Long)] | List[(Agent, Long)]) extends SlanFunction
/*
...
* */
case class Fn_Send(messages2Send: List[(Message, Long)] | Message | Generator) extends SlanFunction
...
* */
case class Fn_Invoke(endpoint: Resource, parameters: Resource) extends SlanFunction
...
case class Behavior(id: EntityId, messagesResponses: List[(Message, List[SlanExpression])]) extends SlanEntity(id)
