...
object Parameters:
  ...
  private def getRange(entry: String): Tuple2[Double, Double] =
    val lst = Try(config.getDoubleList(entry).asScala.toList) match {
      case Success(value) => value.sorted
      case Failure(exception) => logger.error(s"No config parameter is provided: $entry")
