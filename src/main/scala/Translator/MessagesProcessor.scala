...
class MessagesProcessor extends GenericProcessor {
  override protected def yamlContentProcessor(yamlObj: YamlTypes): Eval[SlanConstructs] = yamlObj match {
    case v: (_, _) => convertJ2S(v(0)) match {
      case cv: String => Eval.now(List(Message(List(MessageDeclaration(cv, None)), (new MessageFieldsProcessor).commandProcessor(convertJ2S(v(1))).value)))
      case kv: Map[_, _] => Eval.now(List(Message((new MessageInheritanceKeyProcessor).commandProcessor(convertJ2S(kv)).value, (new MessageFieldsProcessor).commandProcessor(convertJ2S(v(1))).value)))
