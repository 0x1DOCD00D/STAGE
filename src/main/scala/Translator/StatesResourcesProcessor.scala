...
class StatesResourcesProcessor extends GenericProcessor {
  override protected def yamlContentProcessor(yamlObj: YamlTypes): Eval[SlanConstructs] = yamlObj match {
    case v: (_, _) => convertJ2S(v(0)) match {
      ...
      case resources: String if resources.trim.toLowerCase === ResourcesSection.toLowerCase => Eval.now(List(Resources((new AgentLocalResourcesProcessor).commandProcessor(convertJ2S(v(1))).value)))
