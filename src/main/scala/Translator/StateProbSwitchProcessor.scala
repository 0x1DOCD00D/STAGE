...
class StateProbSwitchProcessor extends GenericProcessor:
  override protected def yamlContentProcessor(yamlObj: YamlTypes): Eval[SlanConstructs] = yamlObj match {
    ...
    case v: (_, _) => (convertJ2S(v(0)), convertJ2S(v(1))) match {
      ...
      case (stateId: String, ifBehavior: _) => Eval.now(List(StateConditionalSwitch(Some(stateId.trim), (new BehaviorActionsProcessor).commandProcessor(ifBehavior).value)))
